...
# https://github.com/pranavphoenix/VisionXformer/blob/main/Models/FNet.py
# classes
...
class FNetPooler(nn.Module):
    ...
    def forward(self, x):
        ...
        pooled_output = self.activation(x)
        return pooled_output

# The combination:
class ASTFNetModel(nn.Module):
    """
    The AST model with FNet instead of self-attention transformer.
    :param label_dim: the label dimension, i.e., the number of total classes, it is 527 for AudioSet, 50 for ESC-50, and 35 for speechcommands v2-35
    :param fstride: the stride of patch spliting on the frequency dimension, for 16*16 patchs, fstride=16 means no overlap, fstride=10 means overlap of 6
    :param tstride: the stride of patch spliting on the time dimension, for 16*16 patchs, tstride=16 means no overlap, tstride=10 means overlap of 6
    :param input_fdim: the number of frequency bins of the input spectrogram
...
#                 fnet_dim=768, fnet_depth=4, fnet_mlp_dim=1536, fnet_dropout=0.1):
        ...
        if audioset_pretrain == False:
            ...
            ### FNet
            self.fnet_encoder = FNetEncoder(fnet_dim, fnet_depth, fnet_mlp_dim, fnet_dropout)
            self.fnet_pooler = FNetPooler(fnet_dim)
    ...
    def forward(self, x):
        ...
        ### Pooler
        # take (cls_tokens, dist_token)
        x = (x[:, 0] + x[:, 1]) / 2
        x = self.v.norm(x)

        # fnet pooler
        x = self.fnet_pooler(x)
        ###
